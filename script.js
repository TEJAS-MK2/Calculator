class Calculator {
    ...
    saveHistory() {
        try { localStorage.setItem('calculatorHistory', JSON.stringify(this.history)); } catch { /* storage unavailable */ }
    }
    ...
    renderHistory() {
        ...
        this.historyList.querySelectorAll('[data-history-index]').forEach(item => {
            item.addEventListener('click', () => {
                const entry = this.history[Number(item.dataset.historyIndex)];
                if (!entry) return;
                this.currentInput = entry.result;
                this.previousInput = '';
                this.operator = null;
                this.waitingForOperand = false;
    ...
    toggleHistory(force) {
        ...
        if (open && typeof anime !== 'undefined') {
            anime({ targets: '.history-item, .history-empty', opacity: [0, 1], translateX: [-8, 0], duration: 250, delay: anime.stagger(25), easing: 'easeOutCubic' });
        }
    }
    ...
    scientificFunction(type) {
        ...
        switch (type) {
            ...
            case 'factorial': if (!Number.isInteger(value) || value < 0 || value > 170) return this.showError('Use an integer 0–170'); result = 1; for (let i = 2; i <= value; i++) result *= i; break;
            default: return;
        }
        this.currentInput = String(this.roundResult(result));
        this.justCalculated = true; this.waitingForOperand = false; this.clearSecondaryDisplay(); this.updateDisplay(); this.animateResult();
    }

    insertConstant(value, symbol) {
        this.cancelErrorReset(); this.currentInput = String(this.roundResult(value)); this.justCalculated = true; this.waitingForOperand = false; this.displaySecondary.textContent = symbol; this.updateDisplay();
    }
    ...
    inputNumber(digit) {
        ...
        if (startingNewValue) { this.currentInput = digit; this.waitingForOperand = false; this.justCalculated = false; if (!this.operator) this.clearSecondaryDisplay(); }
    ...
    inputDecimal() {
        this.cancelErrorReset();
        if (this.waitingForOperand || this.justCalculated) { this.currentInput = '0.'; this.waitingForOperand = false; this.justCalculated = false; this.clearSecondaryDisplay(); }
    ...
    handleOperator(nextOperator) {
        ...
        this.waitingForOperand = true; this.operator = nextOperator; this.justCalculated = false; this.updateSecondaryDisplay(); this.updateDisplay();
    }
    ...
    performCalculation() {
        ...
        let result;
        switch (this.operator) { case 'add': result = prev + current; break; case 'subtract': result = prev - current; break; case 'multiply': result = prev * current; break; case 'divide': if (current === 0) { this.showError('Cannot divide by zero'); return null; } result = prev / current; break; default: return null; }
    ...
    calculate() {
        ...
        const left = this.formatNumber(this.previousInput);
        const right = this.formatNumber(this.currentInput);
        ...
        this.currentInput = String(result); this.previousInput = ''; this.operator = null; this.waitingForOperand = false; this.justCalculated = true;
        this.updateDisplay(); this.animateResult();
    }
