...
function tokenize(expression) {
  ...
  while (i < source.length) {
    const c = source[i];
    if (/\d|\./.test(c)) { const m = source.slice(i).match(/^(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?/i); if (!m) throw new Error(`Invalid number near "${source.slice(i)}"`); const value = Number(m[0]); if (!Number.isFinite(value)) throw new Error('Number is outside the supported range'); tokens.push({ type: 'number', value }); i += m[0].length; continue; }
    if (/[A-Za-z_]/.test(c)) { const m = source.slice(i).match(/^[A-Za-z_][A-Za-z0-9_]*/); tokens.push({ type: 'identifier', value: m[0] }); i += m[0].length; continue; }
...
function implicit(tokens) { const out = []; for (let i = 0; i < tokens.length; i++) { const a = tokens[i], b = tokens[i + 1]; out.push(a); if (valueEnd(a) && valueStart(b) && !(a.type === 'identifier' && b.type === '(' && FUNCTIONS[a.value])) out.push({ type: '*', value: '*' }); } return out; }
...
export class Parser {
  ...
  mul() { let v = this.unary(); while (this.peek('*') || this.peek('/') || (this.peek('%') && valueStart(this.tokens[this.position + 1]))) { const op = this.tokens[this.position++].type, r = this.unary(); if (op === '*') v *= r; else if (op === '/') { if (r === 0) throw new EvaluationError('Division by zero', 'DIVISION_BY_ZERO'); v /= r; } else { if (r === 0) throw new EvaluationError('Modulo by zero', 'DIVISION_BY_ZERO'); v %= r; } v = this.finite(v); } return v; }
  unary() { if (this.peek('+')) { this.position++; return this.unary(); } if (this.peek('-')) { this.position++; return -this.unary(); } return this.power(); }
  power() { let v = this.postfix(); if (this.peek('^')) { this.position++; const exponent = this.unary(); v = this.finite(v ** exponent, 'Invalid power operation', 'INVALID_POWER'); } return v; }
  ...
  primary() {
    ...
    if (this.peek('identifier')) { const name = this.consume('identifier').value; if (this.peek('(')) { this.position++; const args = []; if (!this.peek(')')) { args.push(this.add()); while (this.peek(',')) { this.position++; args.push(this.add()); } } this.consume(')'); const fn = FUNCTIONS[name]; if (!fn) throw new EvaluationError(`Unknown function "${name}"`, 'UNKNOWN_FUNCTION'); const expected = name === 'atan2' ? 2 : 1; if ((MULTI.has(name) && args.length < 1) || (!MULTI.has(name) && args.length !== expected)) throw new EvaluationError(`Invalid argument count for ${name}`, 'ARGUMENT_COUNT'); return this.finite(applyFunction(name, fn, args, this.angleMode), `Invalid result from ${name}`, 'INVALID_FUNCTION_RESULT'); } if (Object.hasOwn(CONSTANTS, name)) return CONSTANTS[name]; if (Object.hasOwn(this.scope, name)) { const v = Number(this.scope[name]); if (!Number.isFinite(v)) throw new EvaluationError(`Variable "${name}" is not finite`, 'INVALID_VARIABLE'); return v; } throw new EvaluationError(`Unknown identifier "${name}"`, 'UNKNOWN_IDENTIFIER'); }
...
function applyFunction(name, fn, args, mode) { if (TRIG.has(name)) return fn(toRadians(args[0], mode)); if (INV_TRIG.has(name)) return fromRadians(fn(args[0]), mode); if (name === 'atan2') return fromRadians(fn(args[0], args[1]), mode); if (name === 'sqrt' && args[0] < 0) throw new EvaluationError('Square root requires a non-negative value', 'DOMAIN_ERROR'); if (name === 'acosh' && args[0] < 1) throw new EvaluationError('acosh requires a value of at least 1', 'DOMAIN_ERROR'); if (['log', 'ln', 'log2'].includes(name) && args[0] <= 0) throw new EvaluationError(`${name} requires a valid positive domain`, 'DOMAIN_ERROR'); if (name === 'log1p' && args[0] <= -1) throw new EvaluationError('log1p requires a value greater than -1', 'DOMAIN_ERROR'); return MULTI.has(name) ? fn(...args) : fn(args[0]); }
