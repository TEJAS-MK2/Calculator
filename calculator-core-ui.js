...
(() => {
  ...
  function renderHistory() {
    ...
    for (const item of items) { const button = document.createElement('button'); button.type = 'button'; button.className = 'history-item'; const left = document.createElement('span'), right = document.createElement('span'); left.className = 'history-expression'; right.className = 'history-result'; left.textContent = item.expression; right.textContent = `= ${format(item.result)}`; button.append(left, right); button.addEventListener('click', () => { expression = String(item.result); justCalculated = true; display(format(item.result), 'History'); animateDisplay('result'); }); list.appendChild(button); }
  ...
  function appendDecimal() { if (justCalculated) expression = ''; justCalculated = false; const match = expression.match(/(?:^|[+*/-])(-?\d*\.?\d*)$/), tail = match ? match[1] : ''; if (tail.includes('.')) return; if (!expression || operators.has(expression.at(-1))) expression += '0.'; else expression += '.'; display(expression); animateDisplay(); }
  function appendOperator(operator) { if (justCalculated) justCalculated = false; if (!expression) { if (operator !== '-') return; expression = '-'; display(expression); return; } const last = expression.at(-1); if (operators.has(last)) { if (operator === '-' && last !== '-') expression += '-'; else if (last === '-' && expression.length > 1 && operators.has(expression.at(-2))) { if (operator !== '-') expression = expression.slice(0, -2) + operator; } else expression = expression.slice(0, -1) + operator; } else expression += operator; display(expression); }
  function calculate() {
  ...
  function insertToken(token) { if (justCalculated) expression = ''; justCalculated = false; expression += token; display(expression); animateDisplay(); }
  ...
  function setupEnginePanel() {
    ...
    functions.forEach(token => { const button = document.createElement('button'); button.type = 'button'; button.textContent = token; button.addEventListener('click', () => insertToken(token)); chips?.appendChild(button); });
  ...
  document.addEventListener('keydown', event => { if (event.ctrlKey || event.metaKey || event.altKey) return; if (/^\d$/.test(event.key)) { event.preventDefault(); appendNumber(event.key); return; } if (event.key === '.') { event.preventDefault(); appendDecimal(); return; } if (operators.has(event.key)) { event.preventDefault(); appendOperator(event.key); return; } if (event.key === 'Enter' || event.key === '=') { event.preventDefault(); calculate(); return; } if (event.key === 'Escape') { event.preventDefault(); clear(); return; } if (event.key === 'Backspace') { event.preventDefault(); backspace(); } });
