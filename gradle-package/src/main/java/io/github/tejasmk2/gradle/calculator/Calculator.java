...
/** Advanced decimal and scientific arithmetic engine for Gradle applications. */
public final class Calculator {
    ...
    public static BigDecimal power(BigDecimal a,int exponent){require(a);if(exponent<0&&a.signum()==0)throw new ArithmeticException("cannot raise zero to a negative power");return a.pow(exponent,MC);}
    ...
    public static double nthRoot(double value,int degree){if(degree==0)throw new IllegalArgumentException("root degree cannot be zero");if(value<0&&degree%2==0)throw new IllegalArgumentException("even root requires a non-negative value");return value<0?-Math.pow(-value,1.0/degree):Math.pow(value,1.0/degree);}
